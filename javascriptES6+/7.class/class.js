/*
    ...
    - constructor 생성자를 이용해서 멤버 변수 선언 및 초기화 기능
    - 생성자에서 this를 사용하여 멤버 변수를 지정할 수 있음
    - new 연산자로 인스턴스 생성 가능
*/
// 왕 클래스 : 이름(name) / 묘호(ho) / 생일(birth) / 사망(death)
...
class King {
    // constructor() {}  - 오버로딩 안됨 A class may only have one constructor 오류
...
const jeongjong = new King('이방과', '정조', '1357-07-26', '1419-10-15');
...
// console.log(instance1.#privateField); - 프라이빗으로 설정 해둬서 오류가 발생
...
const chiildInstance = new ChildClass('부모', '자식');
console.log(chiildInstance);
chiildInstance.childMethod();
chiildInstance.parentMethod();
