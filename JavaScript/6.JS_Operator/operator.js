/*
    산술 연산자 : 사직연산을 수행하기 위한 연산자
    - 산술 연산을 할 수 없는 상황이면 NaN 결과를 반환
    - 이항 산술 연산과 단항 산술 연산이 존재
*/

/*
    이항 산술 연산자 : 항이 두개가 필요한 산술 연산자
    - 피연산자를 변경하는 부수 효과가 없음
    - + : 덧셈
    - - : 뺄셈
    - * : 곱셈
    - / : 나눗셈
    - % : 나머지
*/

/*
    단항 산술 연산자 : 항이 하나인 산술 연산자
    - 피연산자를 변경하는 부수 효과가 발생할 수 있음
    - ++ : 증가
    - -- : 감소
    - + : 어떠한 효과도 없음
    - - : 양수를 음수로 음수를 양수로 변경
*/
var single = 0;
var result = 0;

//선 대입 후 증가
result = single++ * 10;  // 0
//선 증가 후 대입
result = ++single * 10;  // 20
//선 대입 후 감소
result = single-- * 10;  // 20
//선 감소 후 대입
result = --single * 10;  // 0

/*
    - + 단항 연산자는 숫자 타입이 아닌 피연산자에 대해서 숫자 타입으로 형변환하는 목적으로 사용
*/
// ! JavaScript 에서만 볼수 있는 방식
console.log(typeof +10);    // number
console.log(typeof +'10'); // number
console.log(+false);       // 0
console.log(+'십');        // NaN

/*
    - - 단항 연산자는 피연산자의 부호를 반전한 결과값을 반환
    - 숫자 타입이 아닌 피연산자에 사용하면 부호가 반전된 숫자 타입으로 변환함
*/
console.log(-10);
console.log(-'10');
console.log(-true);

/*
    문자열 연결 연산자
    - + 이항 연산자의 피연산자 중 하나라도 문자열이면 연결 연산자로 사용됨
*/
console.log(1 + '1');
console.log('나의 나이는 ' + 20 + '살 입니다.');
console.log(10 + 20 + ' 문자열');

/*
    할당 연산자 : 
    -  = : 좌항에 우항을 대입
    - += : 좌항에 좌항의 원래 값과 우항을 더한 값을 대입
    - -= : 좌항에 좌항의 원래 값과 우항을 뺀 값을 대입
    - *= : 좌항에 좌항의 원래 값과 우항을 곱한 값을 대입
    - /= : 좌항에 좌항의 원래 값과 우항을 나눈 값을 대입
    - %= : 좌항에 좌항의 원래 값과 우항을 나눈 후 나머지 값을 대입
*/



/*
    비교 연산자 : 피연산자의 일치 여부 판단 후 값을 반환
    - == : 동등 비교
    - === : 일치 비교
    - != : 부등 비교
    - !== 불일치 비교
*/

// 동등, 부등 비교는 자동 형변환을 수행 후 연산을 진행 (데이터 타입의 영향을 받지 않음)
console.log(10 == '10');
// ! JavaScript 에서만 볼수 있는 방식
// 일치, 불일치 비교는 자동 형변환을 수행 하지않고 연산을 진행 (데이터 타입의 영향을 받음)
console.log(10 === '10');


/*
    대소 관계 비교 연산자 : 피연산자의 크기를 비교하여 논리 값으로 반환
    - > : 좌항이 우항보다 크다
    - < : 좌항이 우항보다 작다
    - >= : 좌항이 우항보다 크거나 같다
    - <= : 좌항이 우항보다 작거나 같다
*/



/*
    삼항 연산자 : 조건표현식의 논리 결과에 따라 서로 다른 결과를 반환
    - 조건표현식 ? true일 경우 결과 : false일 경울 결과
*/
var number = 10;
var result = number % 2 === 1 ? '홀수' : '짝수';
result = (number % 2) ? '홀수' : '짝수';
console.log(result);

/*
    삼항 연산자는 if else 문과 같은 역할을 함
*/
var result = '';
var number = 10;

if (number > 0) {
    result = '양수'
} else if (number < 0) {
    result = '음수'
} else {
    result = '영';
}
// 위에 조건문은 삼항연산자로 했을 경울 밑에 코드
result = (number > 0) ? '양수' : (number < 0) ? '음수' : '영'

/*
    논리 연산자 : 우항과 좌항의 피연산자를 조합
    - || : 좌항과 우항 중 하나라도 true면 true, 그렇지 않으면 false
    - && : 좌항과 우항이 모두 true 이면 true, 그렇지 않으면 false
    - ! : 우항을 부정, 우항이 true이면 false, false이면 true
*/
// ! JavaScript 에서만 볼수 있는 방식 true, false 외의 것도 넣을 수 있는 것
console.log(true || true);
console.log(false && true);
console.log(true || '문자열');
console.log(false || '문자열');  // 문자열 출력
console.log(true && '문자열');  // 문자열 출력
console.log(0 && '문자열');     // 0 출력

/*
    - || 또는 && 연산의 결과가 논리값이 아닐 수 있음
    - 논리 연산 결과를 결정하는 항의 값을 반환
*/

/*
    쉼표 연산자 : 왼쪽 피연산자부터 차례로 연산, 잘 사용 안하는 방식
*/
var x, y, z;
x = 1, y = 2, z = 3;


/*
    typeof 연산자 : 우항의 데이터 및 변수의 타입을 문자열로 반환
*/
var type = typeof x;
console.log(type === 'number');

