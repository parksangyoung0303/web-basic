...
/*
    sort(비교 함수) : 배열의 요소를 정렬
    - .sort()로 정렬하면 사전식 정렬이 이루어짐 (오름차순정렬)
*/
array.sort();
console.log(array);
...
/*
    forEach(콜백 함수) : for문 대신에 사용
    - 배열을 순회하면서 각 요소와 인덱스를 사용할 수 있도록 함
    - 원본 배열 변경X
    - break, continue 등 일반 반복문에 사용되는 부수작을 수행할 수 없음
...
/*
    ...
    - map은 요소나 인덱스를 가지고 작업한 결과물을 반환 (return 반드시 O)
*/
...
/*
    filter(콜백 함수) : 콜백 함수의 반환 값이 true인 배열의 요소의 값만 추출하여 새로운 배열을 생성
    - 배열의 특정 경우만 필터링 조건으로 추출해서 새로운 배열을 만들고 싶을 떄 사용
    - 원본 배열 변경X
*/
